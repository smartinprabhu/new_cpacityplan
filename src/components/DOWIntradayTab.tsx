...
switch (aggregationType) {
  ...
  const renderChart = () => {
    ...
    const showXAxisLabels = chartData.length <= 50;
    const labelInterval = Math.max(0, Math.floor(chartData.length / 15));

    // Calculate Y-axis domain
    const yValues = chartData.map((d) => d.value).concat(overallAverage);
    ...
    const renderLegend = () => (
      <div className="flex items-center justify-center space-x-4 mb-4">
        ...
        <div className="flex items-center">
          ...
          <span className="text-sm text-gray-300">Overall Average ({Math.round(overallAverage).toLocaleString()})</span>
        </div>
      </div>
    );
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', width: '100%', height: '450px' }}>
          {/* Sticky Y-axis */}
          <div style={{ width: '60px', position: 'sticky', left: 0, zIndex: 10, backgroundColor: '#1e293b' }}>
            ...
            <ResponsiveContainer width={chartWidth} height="100%">
              {chartType === 'line' ? (
                <LineChart data={chartDataWithAverage} margin={{ top: 20, right: 30, left: 0, bottom: 60 }}>
                  ...
                  />
                  <YAxis yAxisId="left" orientation="left" stroke="#94a3b8" domain={yDomain} hide={true} />
                  <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569' }} labelStyle={{ color: '#cbd5e1' }} />
                  <Line
                    yAxisId="left"
                    type="monotone"
